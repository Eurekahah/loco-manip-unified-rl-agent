...
class LowLevelPolicyActionBase(ActionTerm):
    ...
    def __init__(self, cfg: ActionTermCfg, env: ManagerBasedRLEnv) -> None:
        ...
        # ── 1. 载入低层策略（清单 ⑦ 的报错在 codex/hl-ckpt-params 里抽成
        #       low_level_replay.load_low_level_policy；本分支先内联，合并后可直接替换）──
        if not check_file_path(cfg.policy_path):
            raise FileNotFoundError(
                f"[{type(self).__name__}] 低层 policy 文件不存在：'{cfg.policy_path}'\n"
                "  可用环境变量 RL_TRAINING_LOW_LEVEL_POLICY_<KEY> 或命令行 "
                "env.actions.<term>.policy_path=... 覆盖；"
                "也可用 export_deploy_policy.py 重新导出。"
            )
        ...
        # 注：codex/hl-ckpt-params 给 verify_low_level_layout 加了 policy_layout_json 参数
        # （会额外打印"actor 输入 = policy_obs + latent"）；两个分支合并后可以把它传进来。
...
def load_low_level_policy_inline(policy_path: str, env, *, tag: str):
    """（兼容用）只做"载入 + 明确报错"的薄封装。"""
    if not check_file_path(policy_path):
        raise FileNotFoundError(f"[{tag}] 低层 policy 文件不存在：'{policy_path}'")
    return torch.jit.load(read_file(policy_path)).to(env.device).eval()
