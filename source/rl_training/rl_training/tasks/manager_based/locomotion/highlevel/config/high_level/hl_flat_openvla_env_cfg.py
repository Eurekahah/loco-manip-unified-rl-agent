...
class HLFlatOpenvlaActionsCfg(HighLevelActionsCfg):
    ...
    openvla_ee_action: mdp.VLAPickActionCfg = mdp.VLAPickActionCfg(
        ...
        vla_device="cuda:0",
        unnorm_key="bridge_orig",            # 未微调时保持此值
        task_instruction="pick up the object",
        infer_every_n_steps=4,
        pos_scale=1.0,
        rot_scale=1.0,
    )
...
class PolicyCfg(HighLevelObservationsCfg.PolicyCfg):
    # openvla中已对图像进行处理，因此不再直接使用原始图像作为观测项
    # 使用预训练视觉编码器
...
class TeacherCfg(HighLevelObservationsCfg.PolicyCfg):
    ...
    def __post_init__(self):
        ...
        self.concatenate_terms = True
@configclass
class HLFlatOpenvlaObservationsCfg(HighLevelObservationsCfg):
    policy: PolicyCfg = PolicyCfg()
    critic: CriticCfg = CriticCfg()
    teacher: TeacherCfg = TeacherCfg()

@configclass
class HLFlatOpenvlaTeacherObservationsCfg(HighLevelObservationsCfg):
    critic: CriticCfg = CriticCfg()
    teacher: TeacherCfg = TeacherCfg()

@configclass
class HLFlatOpenvlaRewardsCfg(HighLevelRewardsCfg):
    """
    抓取任务奖励配置
    - 机器人类型：移动底盘 + 机械臂
    - 末端执行器：arm_link6
    ...
    grasp_contact_finger1 = RewTerm(
        func=mdp.undesired_contacts,         # 正权重 → 鼓励夹爪接触
        weight=1.5,
        params={
...
@configclass
class HLFlatOpenvlaTerminationsCfg(HighLevelTerminationsCfg):
...
@configclass
class HLFlatOpenvlaEventCfg(HighLevelEventCfg):
...
@configclass
class HLFlatOpenvlaEnvCfg(HighLevelFlatEnvCfg):
    actions: HLFlatOpenvlaActionsCfg = HLFlatOpenvlaActionsCfg()
    observations: HLFlatOpenvlaObservationsCfg = HLFlatOpenvlaObservationsCfg()
    rewards: HLFlatOpenvlaRewardsCfg = HLFlatOpenvlaRewardsCfg()
    terminations: HLFlatOpenvlaTerminationsCfg = HLFlatOpenvlaTerminationsCfg()
    commands: HLFlatOpenvlaCommandCfg = HLFlatOpenvlaCommandCfg()
    events: HLFlatOpenvlaEventCfg = HLFlatOpenvlaEventCfg()
